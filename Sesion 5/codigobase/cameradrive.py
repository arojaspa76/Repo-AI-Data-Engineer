...
class streamingmultiplescamaras:
    ...
    def __init__(self, fps=0, camera_type=0, device_id=0, location="https://localhost", flip_screen=0, to_capture_width=1280, 
                 to_capture_height=720, to_display_width=1280, to_display_height=720, enforce_fps=False, debug=False, windows=False):
    ...
    def __gstreamer_pipeline_usb(self, device_name="/dev/video1"):
        return ('v4l2src device=%s ! '
                ...
                'appsink' % (device_name, self.to_capture_width, self.to_capture_height, self.fps))
    ...
    def __gstreamer_pipeline_usb_enforce_fps(self, device_name="/dev/video1"):
        return ('v4l2src device=%s ! '
                ...
                'videoconvert ! '
                'appsink' % (device_name, self.to_capture_width, self.to_capture_height, self.fps, self.fps))
    ...
    def __open_csi(self):
        try:
            ...
            self.cap = cv2.VideoCapture(self.__gstreamer_pipeline_CSI(self.device_id), cv2.CAP_GSTREAMER)

            if not self.cap.isOpened():      # raise an error here, update the error value parameter
        ...
        try:
            if self.debug_mode:
                if self.windows:
                    ...
                    print("rstp GStreamer: ", self.__gstreamer_pipeline_rtsp(self.location))  

            # starts the rtsp client
            if self.windows:
                self.cap = cv2.VideoCapture(self.location)
            else:
                self.cap = cv2.VideoCapture(self.__gstreamer_pipeline_rtsp(self.location), cv2.CAP_GSTREAMER)
            ...
            if self.windows:
                if self.debug_mode:
                     print("Configurando el tamaño de la pantalla: width " + str(self.cap.get(cv2.CAP_PROP_FRAME_WIDTH)) + ", height: " + str(self.cap.get(cv2.CAP_PROP_FRAME_HEIGHT)))
            ...
            if self.enforce_fps:
                if self.debug_mode:
                    if self.windows:
                        ...
                        print("USB GStreamer enforced: ", self.__gstreamer_pipeline_usb_enforce_fps(self.camera_name))
                
                if self.windows:
                    self.cap = cv2.VideoCapture(self.camera_name)
                else:    
                    self.cap = cv2.VideoCapture(self.__gstreamer_pipeline_usb_enforce_fps(self.camera_name), cv2.CAP_GSTREAMER)
                ...
                if self.debug_mode:
                    if self.windows:
                        ...
                        print("USB GStreamer: ", self.__gstreamer_pipeline_usb(self.camera_name))
                
                if self.windows:
                    self.cap = cv2.VideoCapture(int(self.camera_name))
                else:    
                    self.cap = cv2.VideoCapture(self.__gstreamer_pipeline_usb(self.camera_name), cv2.CAP_GSTREAMER)                
